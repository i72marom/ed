...
int ipToInt(IP ip) { //O(1)
	return ip.bytes[0]*(2^24) + ip.bytes[1]*(2^16) + ip.bytes[2]*(2^8) + ip.bytes[3];
}

int nAcc(IP ip, HashTable<IP,int,  IpToInt>& c) { //O(1)
	if (c.find(ip)) return c.get_value();
	return 0;
}

void increment(IP ip, HashTable<IP,int, IpToInt>& c) { //O(1)
	if (c.find(ip)) c.set_value(c.get_value() + 1);
	else c.insert(ip, 1);
}

void decrement(IP ip, HashTable<IP,int, IpToInt>& c) { //O(1)
	if (c.find(ip)) c.set_value(c.get_value() - 1);
	else c.insert(ip, 1);
}
...
//    end.
	...
	while(i < log.size() && (log[i].time < System().time()) ) {
		if (!System().is_banned(log[i].ip)) {
			// TODO
			// update new accesses.
			increment(log[i].ip, counters);
			if (nAcc(log[i].ip, counters) >= max_acc)
